...
test('useQueries fetches multiple queries in parallel', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('[0]:result-a | [1]:result-b'));
...
test('useQueries combine transforms the result', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toContain('Names: Alice, Bob'));
...
test('useQueries select option transforms individual query data', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('84 | $100'));
...
test('useQueries placeholderData shown while loading', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('actual-a | actual-b'));
});
...
test('useQueries with one error and one success', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('OK: ok-data | ERR: test error'));
});
...
test('useQueries reactive query keys trigger re-fetch', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('item-1'));
  expect(fetchHistory).toEqual([1]);

  id(2);
  await flush();

  await waitFor(() => expect(document.body.textContent).toBe('item-2'));
...
test('useQueries subscribed: false returns initial snapshot only', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('initial'));
...
test('useQueries cache updates propagate to all results', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('v1 | x'));

  queryClient.setQueryData(['cache-prop-a'], 'v2');
  await flush();

  await waitFor(() => expect(document.body.textContent).toBe('v2 | x'));
});
...
test('useQueries combine receives reactive properties', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('hasData=true value=42'));
});
...
test('useQueries unmount cleanup removes instances', async () => {
  ...
  await waitFor(() => expect(document.body.textContent).toBe('visible'));
